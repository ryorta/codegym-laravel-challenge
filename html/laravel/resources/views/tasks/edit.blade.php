    ...
    <div>
        ...
        <form method="POST" action="{{ route('tasks.update', ['project' => $project->id, 'task' => $task]) }}">
            ...
            <div class="flex flex-col px-8 pt-6 mx-6 rounded-md bg-white">
                ...
                <div class="-mx-3 md:flex mb-6">
                    <div class="md:w-full px-3 mb-6">
                        <x-label for="text" :value="__('Task Text')" class="{{ $errors->has('text') ? 'text-red-600' :'' }}" /> 
                        <x-textarea id="text" class="block mt-1 w-full {{ $errors->has('text') ? 'border-red-600' :'' }}" type="textarea" name="text" :value="old('text', $task->text)" placeholder="課題の詳細" required autofocus ></x-textarea>
                    </div>
                </div>
